--[[
    ...
    - Fly com Velocity (anti-rollback)
    - Teleport gradual (anti-rollback)
    - Hotkeys personalizáveis
    - Minimize (CTRL restaura), Fechar (confirmação)
...
local FLY_SPEED_MIN, FLY_SPEED_MAX = 10, 50
local flySpeed = 30
...
function createHub()
    ...
    flySpeedLbl.Text = "Velocidade do Fly:"
...
UserInputService.InputBegan:Connect(function(input)
    ...
    if (currentAimbotHotkey and input.KeyCode == currentAimbotHotkey) or (currentAimbotMouseButton and input.UserInputType == currentAimbotMouseButton) then
        aimbotEnabled = not aimbotEnabled
        if hubFrame then
            local minimapanel = hubFrame:FindFirstChildOfClass("Frame")
            local aimPanel = minimapanel and (minimapanel:FindFirstChild("Aimbot: OFF") or minimapanel:FindFirstChild("Aimbot: ON"))
            if aimPanel then aimPanel.Text = aimbotEnabled and "Aimbot: ON" or "Aimbot: OFF" end
        end
        showNotification(aimbotEnabled and "Aimbot ON!" or "Aimbot OFF", aimbotEnabled and "success" or "info")
    end
end)

-- Fly anti-rollback (Velocity)
RunService.RenderStepped:Connect(function()
    if flyEnabled and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if moveVec.Magnitude > 0 then
            hrp.Velocity = moveVec.Unit * math.clamp(flySpeed,FLY_SPEED_MIN,FLY_SPEED_MAX)
        else
    ...
    for _,plr in pairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("Head") then
            ...
            if humanoid and humanoid.Health > 0 then
                ...
                if d < dist then closest = head dist = d end
            end
        end
    end
    return closest
